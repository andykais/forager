...
test('cli basics', async ctx => {
  ...
  await ctx.subtest('search filters', async () => {
    // Set up test data with various attributes
    await forager_cli`--config ${config_path} create ${ctx.resources.media_files['cat_doodle.jpg']} --title "cat drawing" --tags=art`
    forager.media.update(
      forager.media.get({filepath: ctx.resources.media_files['cat_doodle.jpg']}).media_reference.id,
      {stars: 5, view_count: 0}  // unread = true when view_count is 0
    )
    ...
    forager.media.update(
      ...
      {stars: 3, view_count: 1}  // unread = false when view_count > 0
    )
