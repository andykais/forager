...
import { errors, Forager, MediaFileResponse } from "~/mod.ts";
...
test("media actions", async (ctx) => {
  ...
  const media_generated_art = await forager.media.create(
  ...
  ctx.assert.equals(
    media_generated_art.media_file.filepath,
    ctx.resources.media_files["koch.tif"],
  );
  ...
  */
  ...
  let media_cartoon = await forager.media.create(
    ctx.resources.media_files["ed-edd-eddy.png"],
    { title: "Ed Edd Eddy Screengrab" },
    ["cartoon", "wallpaper"],
  );
  ctx.assert.equals(
    media_cartoon.media_file.filepath,
    ctx.resources.media_files["ed-edd-eddy.png"],
  );
  ...
  */
  let media_doodle = await forager.media.create(
  ...
  await ctx.subtest("thumbnail generation", async () => {
    const checksums = {
      generated_art:
        "e00df1e96425e0f231bb0cf065432927933f6f2ffd397119334bd2b0b307923f",
      cartoon:
        "13df03de07b03af9d01667c8a32ef3779a1f3724817b35ffa34909a5d45dc2c6",
      doodle:
        "ee704bb3e4a8ef14bf2825480b4d5e4057be76d4c22386295b7eeaa7278175b2",
    };
    const thumbnails = await Array.fromAsync(
      fs.walk(forager.config.thumbnails.folder),
    );
  ...
  await ctx.subtest("default search arguments", () => {
    ctx.assert.search_result(
      forager.media.search({ cursor: undefined, limit: -1 }),
      {
        total: 3,
        results: [
          { media_reference: { title: "Cat Doodle" } },
  ...
  await ctx.subtest("search pagination", () => {
    ...
    const media_list_page_2 = forager.media.search({
      cursor: media_list_page_1.cursor,
      limit: 2,
    });
  ...
  await ctx.subtest("search filters media_reference_id", () => {
    ctx.assert.search_result(
      forager.media.search({
        query: { media_reference_id: media_generated_art.media_reference.id },
      }),
      {
        total: 1,
        results: [
          {
            media_reference: {
              id: media_generated_art.media_reference.id,
              title: "Generated Art",
            },
          },
        ],
  ...
  await ctx.subtest("search filters tags", () => {
    ctx.assert.search_result(
      forager.media.search({ query: { tags: ["procedural_generation"] } }),
      {
        total: 1,
        results: [{
          media_reference: { id: media_generated_art.media_reference.id },
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { tags: ["wallpaper"] } }),
      {
        total: 2,
        results: [
          { media_reference: { id: media_cartoon.media_reference.id } },
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { tags: ["wallpaper", "colors:black"] } }),
      {
        total: 1,
        results: [
          { media_reference: { id: media_generated_art.media_reference.id } },
    ...
    const wallpaper_media_page_1 = forager.media.search({
      query: { tags: ["wallpaper"] },
      limit: 1,
    });
    ctx.assert.not_equals(wallpaper_media_page_1.cursor, undefined);
    const wallpaper_media_page_2 = forager.media.search({
      query: { tags: ["wallpaper"] },
      limit: 1,
      cursor: wallpaper_media_page_1.cursor,
    });
    ctx.assert.not_equals(wallpaper_media_page_2.cursor, undefined);
    const wallpaper_media_page_3 = forager.media.search({
      query: { tags: ["wallpaper"] },
      limit: 1,
      cursor: wallpaper_media_page_2.cursor,
    });
  ...
  */
  ...
  await ctx.subtest("media update", async () => {
    media_doodle = forager.media.update(media_doodle.media_reference.id, {}, [
      "cat",
    ]);
  ...
  await ctx.subtest("media upsert", async () => {
    ...
    const cronch = await forager.media.upsert(
    ...
    ctx.assert.equals(
      media_cartoon.media_reference.title,
      "Ed Edd Eddy Screengrab",
    );
    media_cartoon = await forager.media.upsert(
      ctx.resources.media_files["ed-edd-eddy.png"],
      { title: "Ed Sparrow" },
      ["cartoon", "wallpaper"],
    );
  ...
  await ctx.subtest("media update remove tag", async () => {
    let cronch = forager.media.get({
      filepath: ctx.resources.media_files["cat_cronch.mp4"],
    });
    ctx.assert.object_match(cronch, {
      tags: [
        { name: "cat", group: "", color: "hsl(0, 70%, 55%)" },
    ...
    cronch = forager.media.update(cronch.media_reference.id, undefined, {
      remove: ["cat"],
    });
    ctx.assert.object_match(cronch, {
      tags: [
...
test("search sort", async (ctx) => {
  ...
  const media_generated_art = await forager.media.create(
    ctx.resources.media_files["koch.tif"],
    { source_created_at: new Date("1/1/2023") },
  );
  const media_cartoon = await forager.media.create(
    ctx.resources.media_files["ed-edd-eddy.png"],
    { source_created_at: new Date("1/1/2024") },
  );
  const media_doodle = await forager.media.create(
    ctx.resources.media_files["cat_doodle.jpg"],
    { source_created_at: new Date("1/1/2025") },
  );

  await ctx.subtest("source_created_at sort order", () => {
    const page_1 = forager.media.search({
    ...
    const page_2 = forager.media.search({
      sort_by: "source_created_at",
      order: "desc",
      limit: 2,
  ...
  await ctx.subtest("created_at sort order", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({ sort_by: "created_at", order: "asc" }),
      {
        results: [
          { media_reference: { id: media_generated_art.media_reference.id } },
          { media_reference: { id: media_cartoon.media_reference.id } },
    ...
    ctx.assert.search_result(
      forager.media.search({ sort_by: "created_at", order: "desc" }),
      {
        results: [
          { media_reference: { id: media_doodle.media_reference.id } },
          { media_reference: { id: media_cartoon.media_reference.id } },
...
test("search sort duration", async (ctx) => {
  ...
  const media_gif = await forager.media.create(
    ctx.resources.media_files["blink.gif"],
  );
  // cat_cronch.mp4 - duration: 6.763
  const media_video = await forager.media.create(
    ctx.resources.media_files["cat_cronch.mp4"],
  );
  // music_snippet.mp3 - duration varies slightly by FFmpeg build (~6.92-6.96)
  const media_audio = await forager.media.create(
  ...
  await ctx.subtest("duration sort order ascending", () => {
    const asc_results = forager.media.search({
      sort_by: "duration",
      order: "asc",
    });
    ...
    ctx.assert.almost_equals(
      asc_media_results[0].media_file.duration,
      3.18,
      0.01,
    );
    ctx.assert.almost_equals(
      asc_media_results[1].media_file.duration,
      6.763,
      0.01,
    );
    ctx.assert.almost_equals(
      asc_media_results[2].media_file.duration,
      6.92,
      0.1,
    );
  });
  ...
  await ctx.subtest("duration sort order descending", () => {
    const desc_results = forager.media.search({
      sort_by: "duration",
      order: "desc",
    });
    ...
    ctx.assert.almost_equals(
      desc_media_results[0].media_file.duration,
      6.92,
      0.1,
    );
    ctx.assert.almost_equals(
      desc_media_results[1].media_file.duration,
      6.763,
      0.01,
    );
    ctx.assert.almost_equals(
      desc_media_results[2].media_file.duration,
      3.18,
      0.01,
    );
  });
  ...
  await ctx.subtest("series excluded from duration sort", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({ sort_by: "duration", order: "asc" }),
      {
        total: 3,
        results: [
          { media_reference: { id: media_gif.media_reference.id } },
  ...
  await ctx.subtest(
    "validate series and duration are mutually exclusive",
    () => {
      ...
      ctx.assert.throws(
        () =>
          forager.media.search({
            query: { series: true, duration: { min: { seconds: 1 } } },
          }),
        errors.BadInputError,
        "Cannot use series filter with duration filter or duration sort",
      );
      ...
      ctx.assert.search_result(
        forager.media.search({ query: { series: true } }),
        {
          total: 1,
          results: [
            { media_reference: { id: test_series.media_reference.id } },
...
test("media search stars", async (ctx) => {
  ...
  const art_1 = await forager.media.create(
    ctx.resources.media_files["koch.tif"],
    { stars: 0 },
  );
  ...
  ctx.assert.search_result(
    forager.media.search({ query: { stars: 1, stars_equality: "gte" } }),
    {
      total: 0,
    },
  );
  ...
  ctx.assert.search_result(
    forager.media.search({ query: { stars: 3, stars_equality: "gte" } }),
    {
      total: 0,
    },
  );
  ctx.assert.search_result(
    forager.media.search({ query: { stars: 3, stars_equality: "eq" } }),
    {
      total: 0,
    },
  );
  ctx.assert.search_result(
    forager.media.search({ query: { stars: 2, stars_equality: "eq" } }),
    {
      total: 1,
    },
  );
...
test("media search duration filter", async (ctx) => {
  ...
  await ctx.subtest("filter with minimum duration", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { duration: { min: { seconds: 6 } } } }),
      {
        total: 2,
        results: [
          { media_reference: { id: media_audio.media_reference.id } },
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { duration: { min: { seconds: 7 } } } }),
      {
        total: 0,
      },
    );
  ...
  await ctx.subtest("filter with maximum duration", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({
        query: { duration: { max: { seconds: 6.763 } } },
      }),
      {
        total: 2,
        results: [
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { duration: { max: { seconds: 3 } } } }),
      {
        total: 0,
      },
    );
  ...
  await ctx.subtest("filter with both min and max duration", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({
        query: { duration: { min: { seconds: 3 }, max: { seconds: 7 } } },
      }),
      {
        total: 3,
        results: [
    ...
    ctx.assert.search_result(
      forager.media.search({
        query: { duration: { min: { seconds: 4 }, max: { seconds: 6.8 } } },
      }),
      {
        total: 1,
        results: [
    ...
    ctx.assert.search_result(
      forager.media.search({
        query: { duration: { min: { seconds: 3.5 }, max: { seconds: 6.5 } } },
      }),
      {
        total: 0,
      },
  ...
  await ctx.subtest("filter with different time units", () => {
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { duration: { min: { minutes: 0.1 } } } }),
      {
        total: 2,
        results: [
          { media_reference: { id: media_audio.media_reference.id } },
    ...
    ctx.assert.search_result(
      forager.media.search({
        query: { duration: { min: { seconds: 3 }, max: { minutes: 0.12 } } },
      }),
      {
        total: 3,
        results: [
...
test("search group by", async (ctx) => {
  ...
  await ctx.subtest("pagination", () => {
    const results_1_2 = forager.media.group({
      group_by: { tag_group: "artist" },
      limit: 2,
    });
    ctx.assert.not_equals(results_1_2.cursor, undefined);
    const results_3 = forager.media.group({
      group_by: { tag_group: "artist" },
      limit: 2,
      cursor: results_1_2.cursor,
    });
      ...
      results: [
        {
          ...
          group: {
            ...
            media: [
              {
                media_reference: { id: succulentsaur.media_reference.id },
              } as any,
        ...
        },
        {
          ...
          group: {
            ...
            media: [
              {
                media_reference: { id: ed_edd_eddy.media_reference.id },
              } as any,
            ],
          },
        },
  ...
  await ctx.subtest("with sort_by / order params", async () => {
    ...
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[0].group.value,
      "alice",
    );
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[0].group.created_at,
      cat_cronch.media_reference.created_at,
    );
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[1].group.value,
      "andrew",
    );
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[1].group.created_at,
      succulentsaur.media_reference.created_at,
    );
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[2].group.value,
      "bob",
    );
    ctx.assert.equals(
      group_sorted_by_created_at_desc.results[2].group.created_at,
      ed_edd_eddy.media_reference.created_at,
    );
    ...
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[0].group.value,
      "andrew",
    );
    ...
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[0].group.created_at,
      generated_art.media_reference.created_at,
    );
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[1].group.value,
      "bob",
    );
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[1].group.created_at,
      ed_edd_eddy.media_reference.created_at,
    );
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[2].group.value,
      "alice",
    );
    ctx.assert.equals(
      group_sorted_by_created_at_asc.results[2].group.created_at,
      cat_cronch.media_reference.created_at,
    );
    ...
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc.results[0].group,
      { value: "bob", last_viewed_at: null },
    );
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc.results[1].group,
      { value: "andrew", last_viewed_at: null },
    );
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc.results[2].group,
      { value: "alice", last_viewed_at: null },
    );

    forager.views.start({ media_reference_id: cat_cronch.media_reference.id });
    cat_cronch = forager.media.get({
      media_reference_id: cat_cronch.media_reference.id,
    }) as MediaFileResponse;
    ctx.assert.not_equals(cat_cronch.media_reference.last_viewed_at, null);
    await ctx.timeout(10);
    forager.views.start({
      media_reference_id: generated_art.media_reference.id,
    });
    generated_art = forager.media.get({
      media_reference_id: generated_art.media_reference.id,
    }) as MediaFileResponse;
    ...
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc_2.results[0].group,
      {
        value: "andrew",
        last_viewed_at: generated_art.media_reference.last_viewed_at,
      },
    );
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc_2.results[1].group,
      {
        value: "alice",
        last_viewed_at: cat_cronch.media_reference.last_viewed_at,
      },
    );
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_desc_2.results[2].group,
      { value: "bob", last_viewed_at: null },
    );
    ...
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_asc.results[0].group,
      {
        value: "alice",
        last_viewed_at: cat_cronch.media_reference.last_viewed_at,
      },
    ...
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_asc.results[1].group,
      {
        value: "andrew",
        last_viewed_at: generated_art.media_reference.last_viewed_at,
      },
    ...
    ctx.assert.object_match(
      group_sorted_by_last_viewed_at_asc.results[2].group,
      { value: "bob", last_viewed_at: null },
    );
  });
  ...
  await ctx.subtest("with grouped_media duration sort", async () => {
    ...
    const alice_group_media_asc = group_with_duration_sort_asc.results[1].group
      .media as MediaFileResponse[];
    ctx.assert.list_deep_partial(
      alice_group_media_asc,
      [
    ...
    ctx.assert.almost_equals(
      alice_group_media_asc[0].media_file.duration,
      6.763,
      0.01,
    );
    ctx.assert.almost_equals(
      alice_group_media_asc[1].media_file.duration,
      6.92,
      0.1,
    );
    ctx.assert.list_deep_partial(
      group_with_duration_sort_asc.results[2].group.media!,
      [
        {
          media_reference: { id: ed_edd_eddy.media_reference.id },
          media_file: { duration: 0 },
        },
      ],
    );
    ...
    ctx.assert.list_deep_partial(
      group_with_duration_sort_desc.results[0].group.media!,
      [
        {
          media_reference: { id: succulentsaur.media_reference.id },
          media_file: { duration: 16.733333 },
        },
        {
          media_reference: { id: cat_doodle.media_reference.id },
          media_file: { duration: 0 },
        },
        {
          media_reference: { id: generated_art.media_reference.id },
          media_file: { duration: 0 },
        },
      ],
    );
    const alice_group_media_desc = group_with_duration_sort_desc.results[1]
      .group
      .media as MediaFileResponse[];
    // same 'artist:alice' tag group as above, media is now sorted by descending duration
    ctx.assert.list_deep_partial(
      alice_group_media_desc,
      [
        { media_reference: { id: music_snippet.media_reference.id } },
        { media_reference: { id: cat_cronch.media_reference.id } },
      ],
    ...
    ctx.assert.almost_equals(
      alice_group_media_desc[0].media_file.duration,
      6.92,
      0.1,
    );
    ctx.assert.almost_equals(
      alice_group_media_desc[1].media_file.duration,
      6.763,
      0.01,
    );
  });
  ...
  await ctx.subtest("with group sort_by duration", async () => {
    ...
    const alice_group = groups_sorted_by_duration_asc.results.find((r) =>
      r.group.value === "alice"
    );
    ...
    const andrew_duration =
      groups_sorted_by_duration_desc.results[0].group.duration;
    const alice_duration =
      groups_sorted_by_duration_desc.results[1].group.duration;
    const bob_duration =
      groups_sorted_by_duration_desc.results[2].group.duration;
...
test("video media", async (ctx) => {
  ...
  let media_cronch = await forager.media.create(
    ctx.resources.media_files["cat_cronch.mp4"],
    {},
    ["cat"],
  );
  ...
  ctx.assert.equals(
    path.basename(media_cronch.thumbnails.results[0].filepath),
    "0001.jpg",
  );

  const media_art_timelapse = await forager.media.create(
    ctx.resources.media_files["Succulentsaur.mp4"],
    {},
    ["art", "timelapse"],
  );
  ...
  ctx.assert.equals(
    media_art_timelapse.thumbnails.results[0].media_timestamp,
    0,
  );
  ...
  const media_cronch_thumbnails_all_search = forager.media.search({
    query: { tags: ["cat"] },
    thumbnail_limit: -1,
  });
  const media_cronch_thumbnails_all =
    media_cronch_thumbnails_all_search.results[0].thumbnails.results;
  ...
  ctx.assert.equals(
    media_cronch_thumbnails_all.at(-1)?.media_timestamp! > 0,
    true,
  );
  ctx.assert.equals(
    media_cronch_thumbnails_all.at(-1)?.media_timestamp! <
      media_cronch.media_file.duration,
    true,
  );
  ...
  await ctx.subtest("keypoints", async () => {
    ...
    const sorted_thumbnails = forager.media
      .get({ media_reference_id: media_cronch.media_reference.id })
      .thumbnails
      .results
      .toSorted((a, b) => a.media_timestamp - b.media_timestamp);
    ...
    expected_thumbnails.forEach((expected_thumbnail, index) => {
      ...
      ctx.assert.almost_equals(
        actual_thumbnail.media_timestamp,
        expected_thumbnail.media_timestamp,
        0.00001,
      );
    });
    ...
    ctx.assert.list_partial(
      forager.media.get({ media_reference_id: media_cronch.media_reference.id })
        .tags,
      [
        { name: "bite" },
        { name: "cat" },
  ...
  await ctx.subtest("keypoint thumbnail file generation", async () => {
    const checksums = {
      cronch:
        "92a575edcc4c5b5b4dd2b3b0908aded951b8c022e0c85ecd6a5a78a4f30fefce",
    };
    const cronch_thumbnails_folder = path.join(
      forager.config.thumbnails.folder,
      "92",
      checksums.cronch,
    );
    const read_thumbnails = await Array.fromAsync(
      fs.walk(cronch_thumbnails_folder),
    );
    const read_thumbnails_sorted = read_thumbnails.map((entry) => entry.path)
  ...
  await ctx.subtest("keypoints in search", async () => {
    ...
    ctx.assert.search_result(forager.media.search(), {
      ...
      results: [
        {
          media_file: {
            filepath: ctx.resources.media_files["Succulentsaur.mp4"],
          },
        },
        {
          media_file: { filepath: ctx.resources.media_files["cat_cronch.mp4"] },
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { keypoint: "bite" } }),
      {
        total: 1,
        results: [{
          media_file: { filepath: ctx.resources.media_files["cat_cronch.mp4"] },
    ...
    ctx.assert.search_result(
      forager.media.search({ query: { tags: ["cat"] } }),
      {
        results: [{
          thumbnails: {
            total: 20,
  ...
  await ctx.subtest("media deletes", async () => {
    ...
    media_cronch = await forager.media.create(
      ctx.resources.media_files["cat_cronch.mp4"],
      {},
      ["cat"],
    );
...
test("search query.animated", async (ctx) => {
  ...
  ctx.assert.search_result(forager.media.search(), {
    results: [
      {
        media_reference: { id: media_video.media_reference.id },
        media_file: { animated: true },
      },
      {
        media_reference: { id: media_doodle.media_reference.id },
        media_file: { animated: false },
      },
      {
        media_reference: { id: media_gif.media_reference.id },
        media_file: { animated: true },
  ...
  ctx.assert.search_result(
    forager.media.search({ query: { animated: true } }),
    {
      results: [
        {
          media_reference: { id: media_video.media_reference.id },
    ...
    results: [
      {
        ...
        media_file: {
          ...
          framerate: 0,
          checksum:
            "1735a26d0182589686bfe0dd9ec4d1e73d82ef7ee95edec3ad6edc9aad48e8d5",
          media_type: "AUDIO",
        },
      },
    ],
...
test("gif", async (ctx) => {
  ...
  const { thumbnails } = forager.media.get({
...
test("media reload", async (ctx) => {
  ...
  const media_image = await forager.media.create(
    ctx.resources.media_files["cat_doodle.jpg"],
    { title: "Cat Doodle" },
    ["doodle"],
  );
  ...
  await ctx.subtest("reload regenerates deleted thumbnails", async () => {
    ...
    const image_result = forager.media.get({
  ...
  await ctx.subtest("reload regenerates video thumbnails", async () => {
    const video_result = forager.media.get({
