...
{#key slug}
  {@const controller = new TagDetailController(props.data.config, slug)}
        ...
        <div class="max-w-3xl mx-auto flex flex-col gap-8">
          ...
          <section>
            ...
            <form class="flex gap-2 items-end" onsubmit={async e => {
              e.preventDefault()
              const input = e.currentTarget.querySelector('input[name="search_bar"]') as HTMLInputElement
              if (input?.value.trim()) {
                await controller.alias_create(input.value.trim())
                input.value = ''
              }
            }}>
              <div class="flex-grow">
                ...
                <TagAutoCompleteInput
                  {controller}
                  bind:search_string={controller.alias_input}
                  kind="details"
          ...
          <section>
            ...
            <div class="mb-4">
              ...
              <form class="flex gap-2 items-end" onsubmit={async e => {
                e.preventDefault()
                if (controller.parent_input.trim()) {
                  await controller.parent_add(controller.parent_input.trim())
                  controller.parent_input = ''
                }
              }}>
                <div class="flex-grow">
                  <TagAutoCompleteInput
                    {controller}
                    bind:search_string={controller.parent_input}
                    kind="details"
            ...
            <div>
              ...
              <form class="flex gap-2 items-end" onsubmit={async e => {
                e.preventDefault()
                if (controller.child_input.trim()) {
                  await controller.child_create(controller.child_input.trim())
                  controller.child_input = ''
                }
              }}>
                <div class="flex-grow">
                  <TagAutoCompleteInput
                    {controller}
                    bind:search_string={controller.child_input}
                    kind="details"
