<script lang="ts">
  ...
  controller.keybinds.component_listen({
    NextMedia: () => {
      ...
      controller.runes.media_selections.next_media(controller.runes.series_list.results)
    },
    PrevMedia: () => {
      if (document.activeElement instanceof HTMLInputElement) {
        if (media_list_element.contains(document.activeElement)) {}
      ...
      controller.runes.media_selections.prev_media(controller.runes.series_list.results)
    },
...
<div
  ...
  <Scroller
    loading={controller.runes.series_list.loading}
    more={() => controller.runes.series_list.paginate()}
    class={[
