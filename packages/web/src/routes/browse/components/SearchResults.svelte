        ...
        >
          ...
          {:else if result.media_type === 'grouped'}
              ...
              <SearchLink
                ...
                {controller} params={queryparams.merge({search_mode: 'media', search_string: `${queryparams.current.group_by ?? ''}:${result.group_metadata.value}`})}> {result.group_metadata.value} 
              </SearchLink>
