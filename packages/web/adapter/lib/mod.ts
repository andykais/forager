...
import {load_config} from './build/config.ts'
...
class Server {
  ...
  async #ensure_static_assets_exist() {
    ...
    if (build_manifest.package_version !== deno_json.version) {
      throw new Error(`unexpected code path. Static asset manifest build.json package version ${build_manifest.package_version} does not match deno.json version ${deno_json.version}`)
    }
