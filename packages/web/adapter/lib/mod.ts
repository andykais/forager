...
import { type Config } from '@forager/server/config'
...
interface ServerOptions {
  ...
  // When provided, /rpc/* and /files/* are dispatched through @forager/server
  // directly, bypassing SvelteKit. SvelteKit still handles all other (HTML)
  // routes. When omitted, requests fall through to SvelteKit as before.
  forager?: Forager
  config?: Config
...
class Server {
  ...
  constructor(options: ServerOptions) {
    ...
    if (options.forager && options.config) {
  ...
  #handle_request = async (request: Request, info: Deno.ServeHandlerInfo<Deno.NetAddr>): Promise<Response> => {
    ...
    */
    ...
    // When the embedder gave us a Forager instance, dispatch /rpc/* and
    // /files/* through @forager/server directly, bypassing SvelteKit.
    if (this.#forager_server) {
