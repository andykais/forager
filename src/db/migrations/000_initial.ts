...
/* CREATE INDEX media_reference_text_search ON media_reference (title) WHERE title IS NOT NULL; */
CREATE INDEX media_file_type ON media_file (media_type, animated);

-- selecting a bunch of thumbnails based on tags
EXPLAIN QUERY PLAN SELECT media_reference.*, media_file.*, media_sequence.* FROM tag
  INNER JOIN media_reference_tag ON tag.id = media_reference_tag.tag_id
  INNER JOIN media_reference     ON media_reference.id = media_reference_tag.media_reference_id
  LEFT  JOIN media_file          ON media_file.media_reference_id = media_reference.id
  LEFT  JOIN media_sequence      ON media_sequence.media_reference_id = media_reference.id
  WHERE tag.name IN ('action', 'danger') AND media_reference.title LIKE 'Neat';


-- this looks weak af. Lets just shove all the tags in the browser. It should be fine?
EXPLAIN QUERY PLAN
SELECT * FROM tag
WHERE name LIKE 'act%';
...
EXPLAIN QUERY PLAN
SELECT * FROM media_sequence
INNER JOIN media_reference ON media_reference.id = media_sequence.media_reference_id
WHERE media_sequence.id = 1
ORDER BY media_reference.media_sequence_index;




SELECT sql FROM sqlite_master WHERE tbl_name IN ('tag');`
